...
<div class="d-flex justify-content-between align-items-center mb-3">
    <div>
        ...
        <div class="text-muted small">
            Clínica: {{ $pedido->clinica->nombre ?? '-' }} · Paciente: {{ ($pedido->paciente->apellido ?? '').' '.($pedido->paciente->nombre ?? '') }}
        </div>
    </div>
    <div>
...
<div class="row">
    ...
    <div class="col-lg-8">
        <div class="card mb-3">
            ...
            <div class="card-body">
                <form method="POST" action="{{ route('admin.tecnico.pedidos.archivos', $pedido) }}" enctype="multipart/form-data">
                    @csrf
                    <div class="form-row">
                        <div class="col-md-4">
                ...
                <div class="table-responsive">
                    <table class="table table-sm mb-0">
                        ...
                        @empty
                            <tr><td colspan="4" class="text-center text-muted py-2">Sin archivos cargados.</td></tr>
                        @endforelse
                        </tbody>
                    </table>
                </div>
        ...
        <div class="card">
            ...
            <div class="card-body">
                <form method="POST" action="{{ route('admin.tecnico.pedidos.fotos', $pedido) }}" enctype="multipart/form-data">
                    @csrf
                    ...
                    <div class="row">
                        @foreach($slots as $slot => $label)
                            <div class="col-md-6 mb-2">
                                ...
                                <input type="file" name="fotos[{{ $slot }}]" class="form-control form-control-sm" accept="image/*">
                            </div>
                        @endforeach
                    </div>
                ...
                <hr>
                <div class="row">
                    @foreach($pedido->fotosRealizadas as $f)
                        <div class="col-md-3 mb-3">
                            <div class="border rounded p-1">
                                <div class="small text-muted">{{ $slots[$f->slot] ?? $f->slot }}</div>
                                <img class="img-fluid"
                                     src="{{ route('admin.resultados.foto.ver', $f) }}"
                                     alt="{{ $f->slot }}">
                            </div>
                        </div>
                    @endforeach
