...
@section('content')
    ...
    <div class="card mb-3">
        <div class="card-body">
            <form class="row g-2 align-items-end" method="get"
                action="{{ route('admin.liquidaciones.pedidos_liquidados') }}">
                <div class="col-md-4">
                    ...
                    <div class="modal-body">
                        ...
                        <div class="form-group">
                            ...
                            <small class="text-muted">
                                Podés cobrar parcial. Si el monto supera el saldo, el excedente queda como pago a cuenta.
                            </small>
                        </div>
    ...
    <script>
        (function() {
            ...
            function fmtGsFromDigits(digits) {
            ...
            function setMontoValueFormatted(inputEl, rawValue) {
                const digits = onlyDigits(rawValue);
                inputEl.value = fmtGsFromDigits(digits);
            }

            const inpMonto = document.getElementById('inpMontoGs');
            const frm = document.getElementById('frmCobrar');

            // Al escribir: solo números y formatea
            if (inpMonto) {

                inpMonto.addEventListener('keydown', function(e) {
                ...
                inpMonto.addEventListener('input', function() {
                    // Mantener cursor razonable (simple, estable)
                    const prev = this.value;
                    setMontoValueFormatted(this, prev);
                });

                inpMonto.addEventListener('paste', function(e) {
                    e.preventDefault();
                    const text = (e.clipboardData || window.clipboardData).getData('text');
                    setMontoValueFormatted(this, text);
                });

                inpMonto.addEventListener('blur', function() {
                    setMontoValueFormatted(this, this.value);
                });
            }

            // Antes de enviar: asegurar formato consistente (solo dígitos -> con puntos)
            if (frm && inpMonto) {
                frm.addEventListener('submit', function() {
                    setMontoValueFormatted(inpMonto, inpMonto.value);
                });
            }

            // Modal: setear valores
            function fmtGs(n) {
                return fmtGsFromDigits(n);
            }
            ...
            document.querySelectorAll('.btn-cobrar').forEach(function(btn) {
                btn.addEventListener('click', function() {
                    ...
                    if (inpMonto) {
                        inpMonto.value = fmtGs(saldo);
                        inpMonto.focus();
                        // opcional: seleccionar todo
                        inpMonto.setSelectionRange(inpMonto.value.length, inpMonto.value.length);
                    }
                });
            });
