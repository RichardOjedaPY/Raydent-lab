...
class ActivityLogController extends Controller
{
    ...
    public function index(Request $r)
    {
        ...
        // ✅ Métricas (con el MISMO filtro)
        $totalFiltered = (clone $filtered)->count();
        $todayFiltered = (clone $filtered)->whereDate('created_at', now()->toDateString())->count();
        $last7Filtered = (clone $filtered)->where('created_at', '>=', now()->subDays(7))->count();
        ...
        // logNames/events deben respetar el mismo scope tenant (sin filtros de búsqueda)
        $logNames = (clone $base)
            ->select('log_name')
            ->distinct()
            ->orderBy('log_name')
        ...
        return view('admin.activity_logs.index', compact(
            'logs', 'logNames', 'events',
            'totalFiltered', 'todayFiltered', 'last7Filtered'
        ));
    }
